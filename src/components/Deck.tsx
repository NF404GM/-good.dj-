...
export const Deck: React.FC<DeckProps> = ({ deckState, dispatch, activeColor }) => {
    ...
    const renderStemPanel = () => {
        const statusMessage = isSeparatingStems
            ? 'Separating stems in the Electron main process.'
            : stemMode === 'real'
                ? 'Real drums, bass, other, and vocals are loaded.'
                : !window.gooddj?.stems
                    ? 'Stem separation is available in Electron only.'
                    : stemModelStatus === null
                        ? 'Checking local stem model status.'
                    : stemModelStatus?.available
                        ? stemModelStatus.message
                        : 'Install or bundle a noncommercial ONNX stem model to enable real stems.';

        const showRealStemStatus = stemMode !== 'real'
            && (!window.gooddj?.stems || !stemModelStatus?.available);
                ...
                <div className="surface-panel rounded-panel flex items-center justify-between gap-4 px-4 py-3">
                    <div className="min-w-0 space-y-1">
                        <div className="font-mono text-[10px] font-black uppercase tracking-[0.22em] text-text-primary">
                            {stemMode === 'real' ? 'Real Stem Playback' : 'Stem Separation'}
                        </div>
                    ...
                    <div className="grid gap-3 xl:grid-cols-3">
                        <div className="surface-panel rounded-panel px-4 py-3">
                            ...
                            <div className="mt-2 font-mono text-[9px] uppercase tracking-[0.16em] text-text-secondary">
                                The controls below shape broad bands until a real ONNX stem model is available.
                            </div>
                        </div>
                        ...
                        <div className="surface-panel rounded-panel px-4 py-3">
                            ...
                            <div className="mt-2 font-mono text-[11px] font-black uppercase tracking-[0.16em] text-text-primary">
                                {stemModelStatus?.available ? 'Ready' : stemModelStatus === null ? 'Checking' : 'Unavailable'}
                            </div>
                            <div className="mt-2 font-mono text-[9px] uppercase tracking-[0.16em] text-text-secondary">
                                {statusMessage}
                            </div>
